...
define(['N/record', 'N/runtime', 'N/search'], (record, runtime, search) => {
    ...
    const createLog = (paramSub, paramPeriod, paramName) => {
        let featSubsidiary = runtime.isFeatureInEffect({ feature: "SUBSIDIARIES" }),
            customRecordAux = record.create({ type: 'customrecord_pe_generation_logs', isDynamic: true });
        ...
        for (var key in nameDataAux) {
            if (nameDataAux.hasOwnProperty(key)) {
                customRecordAux.setValue({ fieldId: key, value: nameDataAux[key] });
            }
        }

        let recordIdAux = customRecordAux.save({ enableSourcing: false, ignoreMandatoryFields: false });
        return recordIdAux;
    }

    const loadLog = (paramRecord, paramName, paramUrl) => {
        let customRecord = record.load({ type: 'customrecord_pe_generation_logs', id: paramRecord });
    ...
    const callFolder = () => {
        var folderSearch = search.load({ id: 'customsearch_pe_search_folder' });
        var folderID
