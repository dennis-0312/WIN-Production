            ...
            if (searchResultCount != 0) {
                if (searchResultCount <= 4000) {
                    searchLoad.run().each((result) => {
                        ...
                        let column03 = result.getValue(searchLoad.columns[2]);//Amount
                        ...
                        let isperson = result.getValue(searchLoad.columns[19]);
                        let name = result.getValue(searchLoad.columns[20]);
                        let apell = result.getValue(searchLoad.columns[21]);

                        log.error('isperson', isperson);
                        log.error('name', name);
                        log.error('apell', apell);
                        //column11 = column11.toFixed(2);
                        column03 = Number(column03);
                        column03 = column03 + '';
                        log.error('column03', column03);
                        if (column03.indexOf('.') != -1) {
                            let div = column03.split('.');

                            if (div[1].length > 2) {
                                column03 = Number(column03).toFixed(2);
                            }
                        } else {
                            column03 = Number(column03);
                        }
                        ...
                        column11 = column11 + '';
                        log.error('column11', column11);
                        if (column11.indexOf('.') != -1) {
                            let div = column11.split('.');

                            if (div[1].length > 2) {
                                column11 = Number(column11).toFixed(2);
                            }
                        } else {
                            column11 = Number(column11);
                        }
                        ...
                        json.push({
                            ...
                            ret_apellidoP: apellidoPaterno,
                            ret_apellidoM: apellidoMaterno,
                            ret_NombreP: NombreP,
                        });
                        return true;
                    });
    ...
    const structureBody = (searchResult) => {
        ...
        try {
            for (let i in searchResult) {
                if((searchResult[i].ruc).substr(0,2) == '20'){
                    contentReport =
                    contentReport +
                    searchResult[i].ruc + '|' +
                    ...
                    + '|\n';
                } else if((searchResult[i].ruc).substr(0,2) == '10'){
                    contentReport =
                    contentReport +
                    searchResult[i].ruc + '|' + '|'  +
                    searchResult[i].ret_apellidoP+ '|' 
                    searchResult[i].ret_apellidoM+ '|' 
                    searchResult[i].ret_NombreP+ '|' +
                    searchResult[i].ret_serie + '|' +
                    searchResult[i].ret_correlativo + '|' +
                    searchResult[i].ref_femision + '|' +
                    searchResult[i].ret_importe + '|' +
