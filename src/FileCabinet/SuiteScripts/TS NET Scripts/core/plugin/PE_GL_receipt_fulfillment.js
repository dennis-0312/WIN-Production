function customizeGlImpact(transactionRecord, standardLines, customLines, book) {
    ...
    if (recType === ITEMRECEIPT || recType === ITEMFULFILLMENT) {
        ...
        try {
            ...
            if (custrecord_pe_suiteglcompras == true || custrecord_pe_suiteglcompras == "T") {
                var paraCuentas20 = []
                nlapiLogExecution('DEBUG', 'MSK', 'Sí aplica');
            ...
            if (recType == ITEMFULFILLMENT) {
                ...
                if (ordertype == "SalesOrd" && formulario == '125' && estado == 'C') {
                    ...
                    for (var j = 0; itemfulfillmentSearch != null && j < itemfulfillmentSearch.length; j++) {
                        var searchresult2 = itemfulfillmentSearch[j];
                        var cuentaItem = searchresult2.getValue('account');
                        var montoFinal = searchresult2.getValue('debitamount');
                        var val_department = searchresult2.getValue('department');
                        var val_class = searchresult2.getValue('class');
                        var val_location = searchresult2.getValue('location');

                        nlapiLogExecution('ERROR', 'MSK', 'cuentaItem=' + cuentaItem);
                        nlapiLogExecution('ERROR', 'MSK', 'montoFinal=' + montoFinal);
                        nlapiLogExecution('ERROR', 'MSK', 'val_department=' + val_department);
                        nlapiLogExecution('ERROR', 'MSK', 'val_class=' + val_class);
                        nlapiLogExecution('ERROR', 'MSK', 'val_location=' + val_location);
                        ...
                        var newLine = customLines.addNewLine();
                        newLine.setCreditAmount(montoFinal);
                        newLine.setAccountId(Number(cuentaItem));
                        //newLine.setMemo(name_item);
                        newLine.setClassId(Number(val_class));
                        newLine.setDepartmentId(Number(val_department));
                        newLine.setLocationId(Number(val_location));
                        ...
                        var newLine2 = customLines.addNewLine();
                        newLine2.setDebitAmount(montoFinal);
                        newLine2.setAccountId(Number(cuentaAjuste));
                        //newLine2.setMemo(name_item);
                        newLine2.setClassId(Number(val_class));
                        newLine2.setDepartmentId(Number(val_department));
